...
data <- suppressMessages(read_excel(here("./data/ROP data_v7.xlsx"),sheet = 2, range = "b3:aT13", col_names = T))
...
text(-5, 3.5, cex=0.7, pos=4, bquote(paste("RE Model for Bevacizumab (Q = ",
...
#text(-5, 23, cex=0.7, pos=4, bquote(paste("RE Model for Aflibercept (Q = ",
...
text(-5, 29.5, cex=0.7, pos=4, bquote(paste("RE Model for Ranibizumab (Q = ",
...
Hence, overall, taking all treatments into account, a predicted 80% (69%-89%) of patients would likely require no retreatment.
...
* Laser effect size (predicted proportion):  65.9% [41.4 - 87.2] ..... (n=`r samples_Laser`)
...
Due to the very high effect sizes associated with all treatments we generally have very high power across all these studies (all in the green so 83+ percent power), i.e. sample sizes are large enough in all studies to detect possible type 2 errors (false negative). 
...
So for example looking at the relative effect table above we can say that the risk of requiring retreatment after laser is 1.686 times the risk of requiring retreatment after Lucentis. (Although note the large confidence intervals which cross RR=1 i.e. no difference)


Note that 1.686 times the risk is $(1.686-1)\times 100 = 68.6\%$ increased risk (but again here this is not a statistically significant result)
...
This suggests Avastin is ranked as the most effective treatment (i.e. lowest risk ratio meaning lowest risk of requiring retreatment) according to the studies involving treatment for zone 1 ROP. Lucentis is ranked second and Laser third. Again these SUCRA rankings are somewhat frowned upon these days as they do not take significance into account in our case only one comparison achieves statistical significance so all other comparisons are void.
