...
data <- suppressMessages(read_excel(("C:\\Users\\ajosan\\Desktop\\R_scripts\\ROP_meta_analysis\\data\\ROP data  21March 2022 Aman FIREFLYEYE.xlsx"),sheet = 2, range = "b3:aT13", col_names = T))
...
forest(metafor_all_res,
      ...
      ilab=cbind(metafor_all_df$ni,
                 metafor_all_df$xi,
                 metafor_all_df$trt),
      ilab.xpos=c(-3.4, -2.7, -1.6), cex = 0.7, ylim=c(-1, 36.5),
      mlab = "",
      order=order(metafor_all_df$trt), 
      rows=c(5:13, 18:26, 31:35),
      xlab="proportion of responders", 
      xlim = c(-5, 3), showweights = T)

text(-4.6, 38, "First author", pos=1,font = 2, cex = .6)
text(-3.4, 38, "No. of px's", pos = 1,font = 2, cex = .6)
text(-2.7, 38, "Responders", pos = 1,font = 2, cex = .6)
text(-1.6, 38, "Treatment", pos = 1,font = 2, cex = .6)
text(1.7, 38, "Weights", pos = 1,font = 2, cex = .6)
text(2.5, 38, "Proportion [95% CI]", pos = 1,font = 2, cex = .6)


### add summary polygons for the four subgroups
addpoly(metafor_Lucentis_res,row=29.5,cex=0.75,transf=transf.ipft.hm, targ=list(ni=metafor_Lucentis_res$ni,xi=metafor_Lucentis_res$xi), mlab="")
addpoly(metafor_Laser_res,row=16.5,cex=0.75,transf=transf.ipft.hm, targ=list(ni=metafor_Laser_res$ni, xi=metafor_Laser_res$xi), mlab="")
...
addpoly(metafor_Avastin_res,row=3.5,cex=0.75,transf=transf.ipft.hm, targ=list(ni=metafor_Avastin_res$ni, xi=metafor_Avastin_res$xi), mlab="")


### add text with Q-value, dfs, p-value, and I^2 statistic for subgroups
text(-5, 3.5, cex=0.7, pos=4, bquote(paste("RE Model for Avastin (Q = ",
...
text(-5, 16.5, cex=0.7, pos=4, bquote(paste("RE Model for Laser (Q = ",
...
text(-5, 29.5, cex=0.7, pos=4, bquote(paste("RE Model for Lucentis (Q = ",
...
Hence, overall, taking all treatments into account, a predicted 81% (70%-90%) of patients would likely require no re-treatment.
...
* Avastin effect size (predicted proportion):  92.7% [85.5 - 98.0]
* Lucentis effect size (predicted proportion):  81.2% [61.1 - 96.0]
* Eylea effect size (predicted proportion):  no studies available
* Laser effect size (predicted proportion):  65.9% [41.4 - 87.2]


<br>


The above for Avastin predicts that treatment with Avastin would likely result in 92.7% of patients requiring no re-treatment. 
...
Due to the very high effect sizes associated with all treatments (80+ percent from the forest plot near the top) we generally have very high power across all these studies (all in the green so 80+ percent power), i.e. sample sizes are large enough in all studies to detect possible type 2 errors (false negative). 


This gives us confidence that statistically significant results are valid but more crucially for us, that all statistically non-significant results are also likely to be valid and not due to insufficient evidence. 
...
So for example looking at the relative effect table above we can say that the risk of requiring re-treatment after laser is 1.92 times the risk of requiring re-treatment after Lucentis. (Although note the large confidence intervals which cross RR=1 i.e. no difference)


Note that 1.92 times the risk is $(1.92-1)\times 100 = 92\%$ increased risk (but again here this is not a statistically significant result)

<br>

*** 
As before the only statistically significant result is that Avastin has become significantly better than Laser at preventing the need for re-treatment for zone 1 ROP. So now the risk of requiring re-treatment after Avastin is 0.30 times the risk of requiring re-treatment after laser and is a statistically significant result 0.3 [0.1 - 0.71]. 
***
...
This suggests Avastin is ranked as the most effective treatment (i.e. lowest risk ratio meaning lowest risk of requiring re-treatment) according to the studies involving treatment for zone 1 ROP. Lucentis is ranked second and Laser third. Again these SUCRA rankings are somewhat frowned upon these days as they do not take significance into account in our case only one comparison achieves statistical significance so all other comparisons are void.
...
In the case of Avastin for example; treatment with Avastin results in 0.3 times the risk of requiring re-treatment as compared to treatment with laser. This is equivalent to saying Avastin results in $(1-0.3) \times 100 = 70\%$ reduction in risk of requiring re-treatment compared to laser.
