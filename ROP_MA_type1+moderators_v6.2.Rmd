---
title: "ROP frequentist meta-analysis - Gestational age, birth weight and postmenstrual age as potential moderators"
author: "Aman Josan"
...
data <- suppressMessages(read_excel(("C:\\Users\\ajosan\\Desktop\\R_scripts\\ROP_meta_analysis\\data\\ROP data  21March 2022 Aman FIREFLYEYE.xlsx"),sheet = 1, range = "b3:ba34", col_names = T))
...
names(df)[names(df) == 'n_responders-Avastin'] <- 'no. responders-Avastin'
names(df)[names(df) == 'n_responders-Lucentis'] <- 'no. responders-Lucentis'
names(df)[names(df) == 'n_responders-Eylea'] <- 'no. responders-Eylea'
...
metafor_Avastin_res_mod <- 
  metafor::rma(yi,vi,mods = ~ g.age+b.weight+pma,data=metafor_all_df, method = "REML", subset=(trt=="Avastin"))

metafor_Lucentis_res_mod <- 
  metafor::rma(yi,vi,mods = ~ g.age+b.weight+pma,data=metafor_all_df, method = "REML", subset=(trt=="Lucentis"))
...
metafor_Eylea_res_mod2 <- 
  metafor::rma(yi,vi,mods = ~ pma,data=metafor_all_df, method = "REML", subset=(trt=="Eylea"))
...
For example for those treated with laser, the p-value for the combined moderating effect of GA, BW & pma is 0.4813 and with individual moderating effect p-values for GA=0.8921 and BW=0.9775 and pma=0.1228.
...
gemtc::forest(relative.effect(mcmc.mr_g.age, t1 = "Lucentis", covariate = weeks))
gemtc::forest(relative.effect(mcmc.mr_g.age, t1 = "Eylea", covariate = weeks))
gemtc::forest(relative.effect(mcmc.mr_g.age, t1 = "Avastin", covariate = weeks))
```
...
```{r, fig.align="center"}
plotCovariateEffect(mcmc.mr_g.age, t1="Avastin", t2="Lucentis", xlim=NULL, ylim=NULL,
...
```{r, fig.align="center"}
plotCovariateEffect(mcmc.mr_g.age, t1="Laser", t2="Lucentis", xlim=NULL, ylim=NULL,
                    ask=dev.interactive(orNone=TRUE))
...
UPDATE (22/03/22) - we seem to have a small band of statistical significance again! Between 25.4 and 27.2 weeks GA, Laser is associated with a statistically significant reduction in risk of requiring retreatment when compared to Lucentis. 
...
```{r, fig.align="center"}
plotCovariateEffect(mcmc.mr_g.age, t1="Laser", t2="Avastin", xlim=NULL, ylim=NULL,
                    ask=dev.interactive(orNone=TRUE))
...
UPDATE (22/03/22) - Again we now have no statististical differences here at any gestational age 
...
```{r, fig.align="center"}
plotCovariateEffect(mcmc.mr_g.age, t1="Laser", t2="Eylea", xlim=NULL, ylim=NULL,
                    ask=dev.interactive(orNone=TRUE))
...
UPDATE (22/03/22) - Between 25.4 and 27.2 weeks GA, Laser is associated with a statistically significant reduction in risk of requiring retreatment when compared to Lucentis. No moderating effect of g.age found on any other treatment modality comparison.
...
df_long2_bw <- df_long2_bw[!(row.names(df_long2_bw) %in% c(32)) , ]
...
gemtc::forest(relative.effect(mcmc.mr_bw, t1 = "Lucentis", covariate = weight))
gemtc::forest(relative.effect(mcmc.mr_bw, t1 = "Eylea", covariate = weight))
gemtc::forest(relative.effect(mcmc.mr_bw, t1 = "Avastin", covariate = weight))
```
...
```{r, fig.align="center"}
plotCovariateEffect(mcmc.mr_bw, t1="Avastin", t2="Lucentis", xlim=NULL, ylim=NULL,
                    ask=dev.interactive(orNone=TRUE))
...
There are no significant differences between treatment outcomes in terms of re-treatment rates between Avastin and Lucentis across various birth weights.

<br>


```{r, fig.align="center"}
plotCovariateEffect(mcmc.mr_bw, t1="Laser", t2="Lucentis", xlim=NULL, ylim=NULL,
                    ask=dev.interactive(orNone=TRUE))
...
UPDATE (22/03/22) - we now a band of statistical significance back. We have a significant moderating effect where birth weights between 800g and 1010g are associated with significant reduction in risk of requiring re-treatment if treated with Laser vs Lucentis
...
```{r, fig.align="center"}
plotCovariateEffect(mcmc.mr_bw, t1="Laser", t2="Avastin", xlim=NULL, ylim=NULL,
                    ask=dev.interactive(orNone=TRUE))
...
There are no significant differences between treatment outcomes in terms of re-treatment rates between Avastin and Laser across various birth weights.
...
```{r, fig.align="center"}
plotCovariateEffect(mcmc.mr_bw, t1="Laser", t2="Eylea", xlim=NULL, ylim=NULL, 
                    ask=dev.interactive(orNone=TRUE))
...
Again, there are no statistically significant differences in treatment outcomes in terms of re-treatment rates evident between Eylea and any other treatment modality.
...
gemtc::forest(relative.effect(mcmc.mr_pma, t1 = "Lucentis", covariate = pma))
gemtc::forest(relative.effect(mcmc.mr_pma, t1 = "Eylea", covariate = pma))
gemtc::forest(relative.effect(mcmc.mr_pma, t1 = "Avastin", covariate = pma))
```
...
```{r, fig.align="center"}
plotCovariateEffect(mcmc.mr_pma, t1="Avastin", t2="Lucentis", xlim=NULL, ylim=NULL,
                    ask=dev.interactive(orNone=TRUE))
...
There are no significant differences between treatment outcomes in terms of re-treatment rates between Avastin and Lucentis across various pma at treatment.

<br>


```{r, fig.align="center"}
plotCovariateEffect(mcmc.mr_pma, t1="Laser", t2="Lucentis", xlim=NULL, ylim=NULL,
                    ask=dev.interactive(orNone=TRUE))
...
We have a statististical differences between Lucentis and Laser when taking mean PMA at treatment into account as a moderator. At low PMA (upto 35.7 weeks) treatment with Laser is associated with a statistically significant reduction in risk of requiring re-treatment than Lucentis. Beyond 35.7 weeks PMA there are no statistical differences.
...
```{r, fig.align="center"}
plotCovariateEffect(mcmc.mr_pma, t1="Laser", t2="Avastin", xlim=NULL, ylim=NULL,
                    ask=dev.interactive(orNone=TRUE))
...
There are no significant differences between treatment outcomes in terms of re-treatment rates between Avastin and Laser across various pma's

<br>


```{r, fig.align="center"}
plotCovariateEffect(mcmc.mr_pma, t1="Laser", t2="Eylea", xlim=NULL, ylim=NULL, 
                    ask=dev.interactive(orNone=TRUE))
...
UPDATE (22/03/22) - We have a new significant result here in that up to 34.3 weeks PMA, there is a statistically significant reduction in risk of requiring re-treatment associated with Laser treatment versus Avastin. After 34.3 weeks there are no differences. 
...
At low PMA (upto 35.7 weeks and 34.3 weeks) treatment with Laser is associated with a statistically significant reduction in risk of requiring re-treatment than Lucentis and Avastin respectively.No other differences are seen at higher PMA or with any other treatment comparisons. 
