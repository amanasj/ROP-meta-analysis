...
addpoly(metafor_Eylea_res,row=35,cex=0.75,transf=transf.ipft.hm, targ=list(ni=metafor_Eylea_res$ni, xi=metafor_Eylea_res$xi), mlab="")
...
#########
text(-5, 79, cex=0.7, pos=4, bquote(paste("RE Model for Lucentis (Q = ",
...
##########
text(-5, 35, cex=0.7, pos=4, bquote(paste("RE Model for Eylea (Q = ",
...
Hence, overall, taking all treatments into account, a predicted 85% (81%-89%) of patients with type 1 ROP would likely require no re-treatment.
...
* Avastin effect size (predicted proportion):  88.0% [80.6 - 93.9]
* Lucentis effect size (predicted proportion):  74.0%  [62.7 - 84.1]
* Eylea effect size (predicted proportion):  80.7%  [62.0 - 94.4] 
* Laser effect size (predicted proportion):  89.9%  [84.6 - 94.3]


<br>


The above for Avastin predicts that treatment with Avastin would likely result in approx 88% of patients with type 1 ROP requiring no re-treatment.
...
Once again all studies have extremely high power (90+ percent) giving us ample confidence that all statistically non-significant results are valid as statements of "no difference" rather than being due to insensitive/insufficient data.
...
Here, we have one comparison that reaches statistical significance. Treatment with Laser has statistically significantly less risk of requiring subsequent re-treatment than after Lucentis treatments. All other comparisons found no evidence of statistically significant differences.
...
* Treatment with Laser is associated with 62% less risk of requiring re-treatment than treatment with Lucentis  for type 1 ROP. This does not take potential moderators into account. We shall look at this later.
<br>
